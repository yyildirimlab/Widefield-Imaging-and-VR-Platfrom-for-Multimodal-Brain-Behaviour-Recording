...
BASE_DIR = "C:\Users\dincb\Desktop\481\processed data\brain\allen\rawBlue_rawViolet_rawHemodynamicSubtr";

% Output directory
OUTPUT_DIR = "C:\Users\dincb\Desktop\481\processed data\rf";
...
try
    pupil_data = load(fullfile("C:\Users\dincb\Desktop\481\processed data\corrBrainBeh\behaviorVars\pupilDia.mat"));
    pupilDia = pupil_data.pupilDia;
    
    speed_data = load(fullfile("C:\Users\dincb\Desktop\481\processed data\corrBrainBeh\behaviorVars\airballSpeed.mat"));
...
%% Visualization: Comparison Across Regions (Normalized 0-1 Scale)
fig = figure('Position', [100 100 1400 800], 'Name', 'Multi-Region RF Performance (Normalized)');

% R² Comparison (already 0-1, but can be negative)
subplot(2,2,1);
...
ylim([0 1]);  % Force 0-1 scale
grid on;

% RMSE Comparison (Normalized)
subplot(2,2,2);
...
% Normalize RMSE to 0-1 (inverse: lower RMSE = better = higher score)
all_rmse = [bilateral_rmse; left_rmse; right_rmse];
max_rmse = max(all_rmse(~isnan(all_rmse)));
min_rmse = min(all_rmse(~isnan(all_rmse)));

bilateral_rmse_norm = 1 - (bilateral_rmse - min_rmse) / (max_rmse - min_rmse);
left_rmse_norm = 1 - (left_rmse - min_rmse) / (max_rmse - min_rmse);
right_rmse_norm = 1 - (right_rmse - min_rmse) / (max_rmse - min_rmse);

bar_data = [bilateral_rmse_norm, left_rmse_norm, right_rmse_norm];
bar(bar_data);
set(gca, 'XTickLabel', brain_regions, 'XTickLabelRotation', 45);
xlabel('Brain Region');
ylabel('Normalized RMSE Score (1=best)');
title('RMSE Comparison (Normalized 0-1)');
legend({'Bilateral', 'Left', 'Right'}, 'Location', 'best');
ylim([0 1]);  % Force 0-1 scale
grid on;
...
% Normalize MAE to 0-1 (inverse: lower MAE = better = higher score)
all_mae = [bilateral_mae; left_mae; right_mae];
max_mae = max(all_mae(~isnan(all_mae)));
min_mae = min(all_mae(~isnan(all_mae)));

bilateral_mae_norm = 1 - (bilateral_mae - min_mae) / (max_mae - min_mae);
left_mae_norm = 1 - (left_mae - min_mae) / (max_mae - min_mae);
right_mae_norm = 1 - (right_mae - min_mae) / (max_mae - min_mae);

bar_data = [bilateral_mae_norm, left_mae_norm, right_mae_norm];
bar(bar_data);
set(gca, 'XTickLabel', brain_regions, 'XTickLabelRotation', 45);
xlabel('Brain Region');
ylabel('Normalized MAE Score (1=best)');
title('MAE Comparison (Normalized 0-1)');
legend({'Bilateral', 'Left', 'Right'}, 'Location', 'best');
ylim([0 1]);  % Force 0-1 scale
grid on;

% CV R² with Error Bars (already 0-1 scale)
subplot(2,2,4);
...
errorbar(x-0.2, bilateral_cv_mean, bilateral_cv_std, 'o-', 'LineWidth', 1.5);
hold on;
...
saveas(fig, fullfile(OUTPUT_DIR, 'MultiRegion_RF_Comparison_Normalized.png'));
fprintf('\n✓ Normalized comparison figure saved to: %s\n', fullfile(OUTPUT_DIR, 'MultiRegion_RF_Comparison_Normalized.png'));